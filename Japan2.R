...
#write.csv(JP1, "JP1.csv")
JP1 <- read.csv("JP1.csv")
...
#これで、日付情報のある人物のデータだけが残る。
DMCdate <- DMCdate %>%
  drop_na("DMC\nAppointment date")    #82233→4121
...
write.csv(DMCdateF_3, "DMCdateF_3.csv")
...
write.csv(JapanDMC, "JapanDMC.csv")
...
library(stargazer)
stargazer(as.data.frame(JP3), type = "text")
...
stargazer(as.data.frame(JP4), type = "text")

write.csv(JP4, "JP4.csv")
...
stargazer(as.data.frame(JP5ind), type = "text")
...
write.csv(JP8_3, "JP8_3.csv")
library(stargazer)
JP8_3<- read.csv("JP8_3.csv")


stargazer(JP8_3, type="text")
...
regs4[["(4)OLS"]] <- ols4
regs4[["(5)2SLS"]] <- iv4
regs4[["(6a)first"]] <- first4_1
regs4[["(6b)first"]] <- first4_2
regs4[["(6c)first"]] <- first4_3
...
modelsummary(regs4, stars = TRUE, 
             gof_omit = "AIC|BIC|Log.Lik.|F|se_type",
             title = "Table4: OLS Regression Estimates of the Relationship Between Firm Performance and the Number of Women Directors on the Board for 4 Industry types in Japan",
             gof_map = c("nobs", "r.squared"),
             add_rows=rows2,
...
regs3[["(4)OLS"]] <- ols3
regs3[["(5)2SLS"]] <- iv3
regs3[["(6a)first"]] <- first3_1
regs3[["(6b)first"]] <- first3_2
regs3[["(6c)first"]] <- first3_3
...
regs7[["(4)OLS"]] <- ols7
regs7[["(5)iv"]] <- iv7
regs7[["(6a)first"]] <- first7_1
regs7[["(6b)first"]] <- first7_2
regs7[["(6c)first"]] <- first7_3
...
regs8[["(7)OLS"]] <- ols8
regs8[["(8)2SLS"]] <- iv8
regs8[["(9a)first"]] <- first8_1
regs8[["(9b)first"]] <- first8_2
regs8[["(9c)first"]] <- first8_3
regs8[["(9d)first"]] <- first8_4
...
modelsummary(regs8, stars = TRUE, 
             ...
             coef_map = c("Femprimary"= "% of female on board * primary",
                          "Femsecondary"= "% of female on board * secondary",
                          "Femtertiary"= "% of female on board * tertiary", 
                          "Femquaternary"= "% of female on board * quaternary",
                          "avg_years2primary"= "(female tenure/boardmembers)*primary", 
                          "avg_years2secondary"="(female tenure/boardmembers)*secondary", 
                          "avg_years2tertiary"="(female tenure/boardmembers)*tertiary",
                          "avg_years2quaternary"="(female tenure/boardmembers)*quaternary"))
